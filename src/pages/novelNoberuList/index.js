...
class MediaList extends PureComponent {
  ...
  getData (value) {
    ...
    dispatch({
      type: 'acgn/novelNoberuListFetch',
      payload: {
        page: value,
        size: NUM_ROWS,
      },
      ...
      this.setState({ mediaList: mediaListTransTo(acgn.novelNoberuList.content) });
      this.setState({ hasMore: !acgn.novelNoberuList.last });
    });
  }
  ...
  render() {
    ...
    return (
      <div>
        <div>
          <CustNavBar title='轻小说列表' pathname={location.pathname} />
        </div>
        <ListView
